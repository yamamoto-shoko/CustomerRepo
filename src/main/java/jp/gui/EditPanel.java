...
public class EditPanel extends JPanel implements ActionListener{
	...
	public EditPanel() {
		...
		JLabel lblNewLabel_5 = new JLabel("取扱状況");
		...
		handlingStatus.addItem("取扱中");
		handlingStatus.addItem("取扱停止中");
		...
		if (e.getActionCommand().equals("編集確認")) {
			...
			try {
				...
				if (result == JOptionPane.YES_OPTION) {
					...
					if (handlingStatus.getSelectedItem().equals("取扱中")) {
	...
	private void searchByItemNumber() {
		...
		if (db.inputtextfield(Integer.parseInt(itemNo.getText())) != null) {
			...
			if(handlingStatusInput.equals("取扱中")) {
				handlingStatus.setSelectedItem("取扱中");
			} else if(handlingStatusInput.equals("取扱停止中")) {
				handlingStatus.setSelectedItem("取扱停止中");
			}
